...
**  ------------------------------------------------------------- */
int transp_connect(struct sockaddr_storage dest, struct sockaddr_storage src) {
...
**  process.
**  ------------------------------------------------------------- */
int intercept_listen(struct sockaddr_storage intercept, struct sockaddr_storage listen_on, int portrange[2]) {
  struct fd_request req;
  int ret;

  write_log(VERBOSE, " TD: intercept_listen(). Setting up req structure");
  req.type = LISTEN;
  req.local = listen_on;
  ...
  do {
    ...
    if (FD_ISSET(listen, &reads)) {
      ...
      case 'F':
        ...
        break;
      case 'N':
        write_log(VERBOSE, "TDS: Accepted new client with fd=%d", fd);
    ...
    for (p = head; p != NULL; p = p->next) {
      if (FD_ISSET(p->fd, &reads)) {
        ...
        if (recv(fd, &p->req, sizeof(p->req), MSG_WAITALL) <= 1) {
          write_log(VERBOSE, "TDS: Closing fd %d", p->fd);
          close(p->fd);
          p->fd = -1;
          continue;
        }
