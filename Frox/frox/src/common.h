...
#define debug_perr(S)                                                                                        \
  fprintf(stderr, "ERROR: \"%s: %s\" at line %d of %s\n", S, strerror(errno), __LINE__, __FILE__);
...
typedef enum _socketuse { ACTV, PASV, CTRL } socketuse;
...
typedef struct _session_info {
  ...
  struct sockaddr_storage apparent_server_address;
  struct sockaddr_storage final_server_address;
  ...
  // FINJAN_START
  sstr *client_ip;
  sstr *client_username;
  sstr *server_hostname;
  int server_hostname_resolved;
  // FINJAN_END
