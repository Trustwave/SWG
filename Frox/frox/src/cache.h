...
#else
static inline int cache_geninit(void) { return (0); };
static inline void cache_init(void){};
static inline int cache_parsed_reply(int code, sstr *msg) { return FALSE; };
static inline void cache_inc_data(sstr *inc){};
static inline int cache_close_data(void) { return -1; };
static inline int cache_transferring(void) { return 0; };
#endif
...
#ifdef USE_LCACHE
int l_geninit(void);
int l_retr_start(const sstr *host, const sstr *file, const sstr *mdtm, int size, int offset, int type);
void l_inc_data(sstr *inc);
int l_retr_end(void);
#else
static inline int l_geninit(void) { return -1; };
