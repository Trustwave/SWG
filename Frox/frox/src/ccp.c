...
static void check_serverok(void) {
  sstr *buf;
  char client_control_ipstr[INET6_ADDRSTRLEN] = {0};
  char server_control_ipstr[INET6_ADDRSTRLEN] = {0};

  buf = sstr_init(MAX_LINE_LEN + 10);

  trustwave_inet_ntop(&info->client_control.address, client_control_ipstr, sizeof(client_control_ipstr));
  trustwave_inet_ntop(&info->server_control.address, server_control_ipstr, sizeof(server_control_ipstr));

  sstr_apprintf(buf, "I %s ", client_control_ipstr);
  sstr_apprintf(buf, "%s %s", server_control_ipstr,
                sstr_len(info->server_name) ? sstr_buf(info->server_name) : "X");

  sstr_apprintf(buf, " %d\n", trustwave_get_port(&info->server_control.address));
...
static int process_reply(sstr *cmd, sstr *arg) {
  ...
  for (;;) {
    ...
    case 'R': /*Redirect */
      ...
      trustwave_inet_pton(sstr_buf(buf), &info->server_control.address);
...
static void ccp_old_init(void) {
  char client_buf[INET6_ADDRSTRLEN] = {0};
  char server_buf[INET6_ADDRSTRLEN] = {0};
  sstr *buf;
  ...
  setenv(EPR "CLIENT", trustwave_inet_ntop(&info->client_control.address, client_buf, sizeof(client_buf)), 1);
  setenv(EPR "SERVER", trustwave_inet_ntop(&info->server_control.address, server_buf, sizeof(server_buf)), 1);
...
static int ccp_old_allowcmd(sstr *cmd, sstr *arg) {
  ...
  if (!sstr_cmp2(cmd, "USER")) {
    setenv(EPR "SERVER", trustwave_inet_ntop(&info->server_control.address, server_ip, sizeof(server_ip)), 1);
    setenv(EPR "SERVERNAME", sstr_buf(info->server_name), 1);
