...
static struct cmd_struct cache_list[] = {{"MODE", cache_mode},
                                         {"STRU", cache_stru},
                                         {"TYPE", cache_type},
                                         {"REST", cache_rest},
                                         {"RETR", cache_retr},
...
static struct cmd_struct xfer_list[] = {
    {"ABOR", cache_abor}, {"STAT", cache_stat}, {"", cache_notallowed}, {"", NULL}};

static int (*retr_start)(const sstr *host, const sstr *file, const sstr *mdtm, int size, int offset,
                         int type);
...
void cache_type(sstr *cmd, sstr *arg) {
  ...
  } else if (!sstr_casecmp2(arg, "AN") || !sstr_casecmp2(arg, "A")) {
    type = 0;
    send_command(cmd, arg);
  } else
...
void cache_notallowed(sstr *cmd, sstr *arg) {
  write_log(ERROR, "Command %s not allowed during cache xfer", sstr_buf(cmd));
...
** ------------------------------------------------------------- */
void cache_stat(sstr *cmd, sstr *arg) { send_cmessage(213, "Retrieving file through cache"); }
...
void cache_abor(sstr *cmd, sstr *arg) {
  ...
  if (info->server_control.fd == -1)
    die(INFO, "Server timed out during cache transfer", 421, "Sorry - timed out", 0);
}
...
void cache_retr(sstr *cmd, sstr *arg) {
  ...
  if (setup_fileinfo(arg) == 0)
    i = retr_start(fileinfo.host, fileinfo.path, fileinfo.mdtm, fileinfo.size, offset, type);
  ...
  if (info->client_data.fd == -1 && info->mode != PASSIVE) {
    if (config.transdata) {
      struct sockaddr_storage tmp = info->apparent_server_address;
      trustwave_set_port(&tmp, 20);
      info->client_data.fd = transp_connect(info->client_data.address, tmp);
    } else
      info->client_data.fd =
          connect_to_socket(&info->client_data.address, &config.tcpoutaddr, config.actvports);
...
int setup_fileinfo(sstr *filename) {
  int code;
  sstr *msg;
  char final_server_ipstr[INET6_ADDRSTRLEN] = {0};

  if (!strcasecmp(config.cachemod, "Local") || !config.forcehttp) {
    send_ccommand("SIZE", sstr_buf(filename));
    get_message(&code, &msg);
    if (code / 100 != 2) {
      write_log(VERBOSE, "SIZE not accepted - aborting caching");
      return (-1);
    }
    ...
    get_message(&code, &msg);
    if (code / 100 != 2) {
      write_log(VERBOSE, "MDTM not accepted - aborting caching");
      return (-1);
    }
    sstr_cpy(fileinfo.mdtm, msg);
    write_log(VERBOSE, "Cache: MDTM is %s", sstr_buf(fileinfo.mdtm));
  }
  ...
  if (!config.strictcache) {
    if (sstr_getchar(filename, 0) != '/') {
      ...
      if (sstr_getchar(fileinfo.path, sstr_len(fileinfo.path) - 1) != '/')
  ...
  if (config.usefqdn && sstr_len(info->server_name)) {
    ...
    trustwave_inet_ntop(&info->final_server_address, final_server_ipstr, INET6_ADDRSTRLEN);
    sstr_cpy2(fileinfo.host, final_server_ipstr);
  }

  if (trustwave_get_port(&info->final_server_address) != 21)
    sstr_apprintf(fileinfo.host, ":%d", trustwave_get_port(&info->final_server_address));
...
int cache_parsed_reply(int code, sstr *msg) {
  ...
  if (working && code == 421) {
    write_log(VERBOSE, "Discarding server close during cache retrieval");
    return TRUE;
  }
  return FALSE;
}
...
void cache_inc_data(sstr *buf) {
  ...
  if (nooping && time(NULL) - last_noop > NOOP_INTERVAL && info->server_control.fd != -1) {
...
int cache_transferring(void) { return working; }
