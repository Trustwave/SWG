...
void client_data_connect() {
  socklen_t len = sizeof(info->client_data.address);

  write_log(VERBOSE, "  Client has connected to proxy data line");
  info->client_data.fd = accept(info->client_listen, (struct sockaddr *)&info->client_data.address, &len);

  if (config.sameaddress) {

    if (!trustwave_is_same_addr((struct sockaddr *)&info->client_data.address,
                                (struct sockaddr *)&info->client_control.address)) {
      char client_buffer[INET6_ADDRSTRLEN] = {0};
      write_log(ATTACK, "Blocked %s from connecting to data line",
                trustwave_inet_ntop(&info->client_data.address, client_buffer, INET6_ADDRSTRLEN));
...
void server_data_connect() {
  socklen_t len = sizeof(info->server_data.address);

  write_log(VERBOSE, "  Server has connected to proxy data line");
  info->server_data.fd = accept(info->server_listen, (struct sockaddr *)&info->server_data.address, &len);
  if (config.sameaddress) {
    if (!trustwave_is_same_addr((struct sockaddr *)&info->server_data.address,
                                (struct sockaddr *)&info->server_control.address)) {
...
int connect_server_data() {
  info->server_data.fd = connect_to_socket(&info->server_data.address, &config.tcpoutaddr, config.pasvports);
  if (info->server_data.fd == -1) {
...
int connect_client_data() {
  ...
  if (config.transdata)
    info->client_data.fd = transp_connect(info->client_data.address, info->server_data.address);
  else {
    struct sockaddr_storage *addr = 0;

    if (1 == config.listeners) { // default to ipv4
      write_log(VERBOSE, "singular listener via ipv4");
      addr = config.listen4.sin_addr.s_addr ? (struct sockaddr_storage *)&config.listen4 : 0;
      info->client_data.fd = connect_to_socket(&info->client_data.address, addr, config.actvlocalports);
    } else { // more then one listener
      if (AF_INET6 == info->client_data.address.ss_family) {
        write_log(VERBOSE, "try listener ipv6");
        addr = config.listen6.sin6_addr.s6_addr ? (struct sockaddr_storage *)&config.listen6 : 0;
        info->client_data.fd = connect_to_socket(&info->client_data.address, addr, config.actvlocalports);
      } else {
        write_log(VERBOSE, "try listener ipv4");
        addr = config.listen4.sin_addr.s_addr ? (struct sockaddr_storage *)&config.listen4 : 0;
        info->client_data.fd = connect_to_socket(&info->client_data.address, addr, config.actvlocalports);
      }
    }
...
void forwarddata2server() {
  ...
  if (info->state != UPLOAD)
    ...
    //			cache_inc_data(info->server_data.buf);
    //			vscan_inc(info->server_data.buf);
    //		} else {		/*local caching */
    vscan_inc(info->server_data.buf);
    //			cache_inc_data(info->server_data.buf);
    //		}
...
void forwarddata2client() {
  ...
  if (info->ssl_sd)
    i = ssl_append_read(info->ssl_sd, info->client_data.buf, dlrate);
  else
    i = sstr_append_read(info->server_data.fd, info->client_data.buf, dlrate);
...
void bwlimit(int bytes, int rate) {
  ...
  actualtime = (now.tv_sec - last.tv_sec) + (double)(now.tv_usec - last.tv_usec) / (double)1000000;
  ...
  if (actualtime < mintime) {
    ...
    ts.tv_nsec = (long)((sleep_time - ts.tv_sec) * (double)1000000000);
    nanosleep(&ts, &ts);
