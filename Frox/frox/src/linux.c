...
static enum { LINUX_2_0, LINUX_2_2, LINUX_2_4, OTHER } kernel;
...
int get_orig_dest(int fd, struct sockaddr_storage *addr) {
  ...
  default:
    ...
    if (getsockopt(fd, SOL_IP, SO_ORIGINAL_DST, (struct sockaddr *)addr, &len))
      return -1;
    if (!trustwave_inet_ntop(addr, addrbuf, sizeof(addrbuf)))
      return -1;
    return 0;
#else
    write_log(ERROR, "Running on a kernel we haven't been compiled for. Oooops.");
    return (-1);
#endif
  }
}
...
int get_local_address6(const int fd, struct sockaddr_in6 *addr) {
  ...
  *addr = *(struct sockaddr_in6 *)&config.secondary_addr;
  if (addr->sin6_addr.s6_addr != 0) {
    addr->sin6_port = 0;
    return 0;
  }
  switch (kernel) {
  case LINUX_2_2:
    ...
    int af = (int)((struct sockaddr_in6 *)&info->client_control.address)->sin6_family;

    if (AF_INET6 == af) {
      addr->sin6_family = AF_INET6;
      addr->sin6_addr = ((struct sockaddr_in6 *)&info->client_control.address)->sin6_addr;
...
int get_local_address(const int fd, struct sockaddr_in *addr) {
  ...
  *addr = *(struct sockaddr_in *)&config.primary_addr;
  ...
  case LINUX_2_2:
    ...
    int af = (int)((struct sockaddr_in *)&info->client_control.address)->sin_family;

    if (AF_INET == af) {
      addr->sin_family = AF_INET;
      addr->sin_addr = ((struct sockaddr_in *)&info->client_control.address)->sin_addr;
...
int bindtodevice(int fd) {
  if (!config.device)
    return (0);
  if (setsockopt(fd, SOL_SOCKET, SO_BINDTODEVICE, (void *)config.device,
...
int kernel_transdata_setup() {
  ...
  fprintf(stderr, "You appear to be running a 2.4.x Linux kernel,"
...
int kernel_td_connect(struct fd_request req) {
  ...
  uid = geteuid();
  write_log(VERBOSE, "TDS: Regaining priveliges for bind-to-foreign-address");
  seteuid(0);
  ...
  i = connect(sockfd, (struct sockaddr *)&req.remote, sizeof(req.remote));
...
int kernel_td_listen(struct fd_request req) {
  ...
  uid = geteuid();
  write_log(VERBOSE, "TDS: Regaining privelidges for bind-to-foreign-address");
  seteuid(0);
...
int kernel_td_unlisten(struct fd_request req) { return (0); }
...
struct ipt_entry *get_entry(struct sockaddr_storage src, struct sockaddr_storage dst,
                            struct sockaddr_storage to, int snat);
...
int kernel_td_connect(struct fd_request req) {
  ...
  /* DO SNAT */
  address = config.primary_addr;
  trustwave_set_port(&address, 0);
...
int add_entry(const struct ipt_entry *e, const char *chain) {
  ...
  uid = geteuid();
  write_log(VERBOSE, "TDS: Regaining privelidges for inserting firewall rules");
  seteuid(0);

  if ((h = iptc_init("nat")) && iptc_append_entry(chain, e, &h) && iptc_commit(&h))
    ret = 0;
  else
    ret = -1;
...
int delete_entry(const struct ipt_entry *e, const char *chain) {
  ...
  uid = geteuid();
  write_log(VERBOSE, "TDS: Regaining privelidges for deleting firewall rules");
  seteuid(0);

  if ((h = iptc_init("nat")) && (matchmask = malloc(e->next_offset)) &&
...
struct ipt_entry *get_entry(struct sockaddr_storage src, struct sockaddr_storage dst,
	...
	size3 = IPT_ALIGN(sizeof(struct ipt_entry_target) +
			  sizeof(struct nf_nat_multi_range));
...
//
	...
	if((e->ip.src = src.sin) == INADDR_ANY)
		e->ip.smsk.s_addr = 0;
	else
