...
static session_info *init_info(int fd, struct sockaddr_storage source);
...
**  session info.
** ------------------------------------------------------------- */
void init_session(int fd, struct sockaddr_storage source) {
  char server_ip[INET6_ADDRSTRLEN] = {0};

  info = init_info(fd, source);

  write_log(INFO, "Connect from: %s", sstr_buf(addr2name((struct sockaddr *)&info->client_control.address)));

  /*FIXME have a login function which deals with ntp and cache */
  ccp_changedest();
  ntp_changedest();
  memcpy(&info->final_server_address, &info->server_control.address, sizeof(info->server_control.address));
  trustwave_inet_ntop(&info->final_server_address, server_ip, sizeof(server_ip));
  write_log(INFO, "... to %s(%s)", server_ip, sstr_buf(info->server_name));

#ifdef ENABLE_CHANGEPROC
  set_proc_title("frox: %s <-> %s", trustwave_inet_ntop(info->client_control.address),
                 trustwave_inet_ntop(info->final_server_address));
#endif
  connect_to_server();
...
static void connect_to_server(void) {
  ...
  uint16_t server_control_port = trustwave_get_port(&info->server_control.address);
  uint16_t listener_port = trustwave_get_port(&config.primary_addr);

  /*Check for loops. Won't work if Listen undefined */
  if ((0 == sockaddr_cmp_addr(&info->server_control.address, sizeof(info->server_control.address),
                              &config.primary_addr, sizeof(config.primary_addr))) &&
      server_control_port == listener_port)
    die(ERROR, "Attempt to connect to self. "
               "Do you need to set DoNTP to yes?",
        421, "Proxy tried to loop. Closing connection", 0);

  if (addr_is_any(&info->server_control.address, sizeof(struct sockaddr_storage))) {
    if (!config.ntp)
      die(ERROR, "Frox unable to determine destination address. "
                 "Do you need to set DoNTP to yes?",
          501, "Unable to contact server", 0);
    else if (0 != addr_is_any(&config.ntpdest, sizeof(struct sockaddr_storage)))
      die(ERROR, "Frox unable to determine destination address. "
                 "Try commenting out NTPAddress",
          501, "Unable to contact server", 0);
    else
      die(ERROR, "Frox unable to determine detination address. "
                 "See FAQ for troubleshooting.",
          501, "Unable to contact server", 0);
  }

  resolve_addr(&info->final_server_address, info->server_name);

  if (info->server_hostname_resolved == 0) // (transparent case) - server_hostname holds IP address
  {
    sstr_cpy(info->server_hostname, info->server_name);
  }

  /* [WARN] this is a hack for dealing with ACLs in ipv6 */
  struct sockaddr_storage client, server;
  memcpy(&client, &info->client_control.address, sizeof(client));
  memcpy(&server, &info->final_server_address, sizeof(server));
  if (AF_INET6 == info->client_control.address.ss_family) {
    struct sockaddr_in tmp;
    memset(&tmp, 0, sizeof(tmp));
    tmp.sin_family = AF_INET;
    inet_pton(AF_INET, "127.0.0.1", &tmp.sin_addr);
    tmp.sin_port = ((struct sockaddr_in6 *)&client)->sin6_port;
    memset(&client, 0, sizeof(client));

    memcpy(&client, &tmp, sizeof(tmp));
    tmp.sin_port = ((struct sockaddr_in6 *)&server)->sin6_port;
    memset(&server, 0, sizeof(server));
    memcpy(&server, &tmp, sizeof(tmp));
  }
  /*[end note]*/
  if (!config_connectionok((struct sockaddr_in *)&client, (struct sockaddr_in *)&server,
                           info->username ? sstr_buf(info->username) : 0))
    die(ATTACK, "Denied by ACLs.", 501, "Connection denied. Bye", 0);

  if (trustwave_inet_ntop(&config.ftpproxy, ftpbuffer, INET6_ADDRSTRLEN)) {

    write_log(VERBOSE, "Copying ftpproxy address into server_control.address ...");
    memcpy(&info->server_control.address, &config.ftpproxy, sizeof(config.ftpproxy));
  }

  write_log(VERBOSE, "Connecting to server...");

  info->server_control.fd =
      connect_to_socket(&info->server_control.address, &config.tcpoutaddr, config.contports);

  if (info->server_control.fd == -1)
    die(ERROR, "Connection closed -- unable to contact server", 501, "Proxy unable to contact ftp server", 0);

  write_log(VERBOSE, "OK");

  if (config.loglevel >= VERBOSE) { /*Save the overhead of DNS lookups */
    write_log(VERBOSE, "Apparent address = %s",
              sstr_buf(addr2name((struct sockaddr *)&info->apparent_server_address)));
    write_log(VERBOSE, "Real address = %s",
              sstr_buf(addr2name((struct sockaddr *)&info->final_server_address)));
    write_log(VERBOSE, "Proxy address = %s",
              sstr_buf(addr2name((struct sockaddr *)&info->server_control.address)));
  }
  ssl_init();
}
...
**  ------------------------------------------------------------- */
static session_info *init_info(int fd, struct sockaddr_storage source) {
  char s1[INET6_ADDRSTRLEN] = {0};
  char s2[INET6_ADDRSTRLEN] = {0};
  ...
  // FINJAN_START
  i->client_ip = sstr_init(0);
  i->client_username = sstr_init(0);
  i->server_hostname = sstr_init(0);
  i->server_hostname_resolved = 0;
  // FINJAN_END
  ...
  i->state = NEITHER;
  i->greeting = AWAITED;
  trustwave_inet_ntop(&source, s1, sizeof(s1));
  write_log(VERBOSE, "source address: %s", s1);

  trustwave_inet_ntop(&i->client_control.address, s2, sizeof(s2));
  write_log(VERBOSE, " client control address: %s", s2);

  memcpy(&i->client_control.address, &source, sizeof(source));
  trustwave_inet_ntop(&i->client_control.address, s2, sizeof(s2));
  write_log(VERBOSE, "AFTER copy,  source address %s to client_control address: %s", s1, s2);
  i->client_control.fd = fd;

  get_orig_dest(fd, &i->server_control.address);
  memcpy(&i->apparent_server_address, &i->server_control.address, sizeof(i->server_control.address));
...
void client_control_forward() {
  ...
  while ((i = extract_clientcmd(info->client_control.buf, &cmd, &arg)) == 0)
    parse_client_cmd(cmd, arg);

  if (i < 0)
    die(ATTACK, "Client is sending us a badly formed control stream.", 421,
        "You are sending me rubbish. Goodbye.", -1);
}
...
void server_control_forward() {
  ...
  while ((i = extract_servercmd(info->server_control.buf, &code, &arg)) == 0)
    if (!ssl_parsed_reply(code, arg) && /*Always returns 0 */
        !cache_parsed_reply(code, arg) && !vscan_parsed_reply(code, arg) &&
        !block_noop_reply(code, arg))
      send_message(code, arg);

  if (i < 0)
    die(ATTACK, "Server is sending us a badly formed control stream.", 421,
        "FTP server is sending you rubbish! Closing connection.", -1);
}
...
void get_message(int *code, sstr **msg) {
  do {
    ...
    case -1:
      die(ATTACK, "Server is sending us a badly formed control stream.", 421,
          "FTP server is sending you rubbish!"
          "Closing connection.",
          -1);
    case 0:
#ifdef DEBUG
    ...
    case 1:
      if (get_control_line(GET_SRVR) <= 0)
        die(ERROR, "Arrghh - shoudln't be here", 0, NULL, -1);
    }
  } while (TRUE);
}
...
int read_srvrctrl_data(void) {
  if (info->ssl_sc)
    return ssl_append_read(info->ssl_sc, info->server_control.buf, 0);
  else
    return sstr_append_read(info->server_control.fd, info->server_control.buf, 0);
}
...
int get_control_line(int which) {
  ...
  do {
    ...
    if (FD_ISSET(info->client_control.fd, &reads)) {
      i = sstr_append_read(info->client_control.fd, info->client_control.buf, 0);
      if (i == 0)
        die(INFO, "Client closed connection", 0, NULL, 0);
      if (i < 0)
        die(ATTACK, "Client flooding control connection", 421, "You're sending rubbish. Goodbye", -1);
      if (sstr_hasline(info->client_control.buf))
        ret |= GET_CLNT;
    }

    if (info->server_control.fd != -1 && FD_ISSET(info->server_control.fd, &reads)) {
      i = read_srvrctrl_data();
      if (i == 0) {
        if (!cache_transferring())
          die(ERROR, "Server closed the control connection", 0, NULL, 0);
        else {
          rclose(&info->server_control.fd);
          write_log(INFO, "Server closed connection. Keeping going until cache done");
        }
      }
      if (i < 0)
        die(ATTACK, "Server flooding the control connection", 421, "Server is sending rubbish."
                                                                   "Closing connection",
            -1);
...
int extract_servercmd(sstr *buf, int *code, sstr **pmsg) {
  ...
  if (!multiline || (sstr_atoi(msg) == multiline && sstr_getchar(msg, 3) == ' ')) {
    multiline = 0;
