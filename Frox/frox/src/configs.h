...
struct options {
  ...
  int listeners; // num of listeners 1 or 2 { 1 = ipv4, 2 = ipv4, ipv6 }
  int lport;
  char *resolvhack;
  char *device;
  int inetd;
  int nodetach;
  struct sockaddr_storage ftpproxy;
  int ftpproxynp;
  int contports[2];
  int pasvports[2];
  int actvports[2];
  int actvlocalports[2];
  int timeout;
  int maxforks;
  int maxforksph;
  int maxdlrate;
  ...
  int paconv;
  int ntp;
  struct sockaddr_storage ntpdest;
  struct sockaddr_storage tcpoutaddr;
  ...
  gid_t gid;
  int fakentp;
  struct vscan_info vscan_limitaions[2];
};
