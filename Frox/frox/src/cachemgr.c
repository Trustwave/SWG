...
static int parse_urireq(sstr *req, sstr *uri, sstr *mdtm, int *size, int *offset, int *type);
static int get_cache_status(int *retfd, sstr *uri, sstr *mdtm, int size, int offset);
...
static int get_cache_status(int *retfd, sstr *uri, sstr *mdtm, int size, int offset) {
  ...
  if (sstr_cmp(mdtm, p->mdtm)) {
    write_log(VERBOSE, "Cache file hit, but remote file has changed");
    purge_entry(p);
    return CACHE_MISS;
  }
  if (p->size != size) {
    write_log(VERBOSE, "Ooops. Cache file hit, MDTM hasn't changed,"
                       "but SIZE has.");
    purge_entry(p);
    return CACHE_MISS;
  }
  ...
  if ((*retfd = open_cachefile(p, offset)) == -1) {
    write_log(VERBOSE, "Cache hit, but offset > cached bytes. Can't cache.");
    return CACHE_ABORT;
  }

  if (set_read_lock(*retfd) == -1) {
    write_log(VERBOSE, "Cache hit, but can't get read lock on file."
                       "Aborting");
    rclose(retfd);
    return CACHE_ABORT;
  }
...
int make_cachefile(struct cache_entry *p) {
  ...
  sstr_apprintf(buf, "%03d  %s %012d %01d %s %012lu\n", sstr_len(p->mdtm) + sstr_len(p->uri) + 30,
                sstr_buf(p->mdtm), p->size, 1, sstr_buf(p->uri), p->last_access);
...
struct cache_entry **hash_loc(sstr *key) {
  ...
  /*Collision */
  for (p = hash_table[i]; p->collision != NULL && sstr_cmp(p->collision->uri, key); p = p->collision)
    ;
  return (&p->collision);
}
...
void add_entry(struct cache_entry *entry) {
  ...
  if (*pp != NULL) {
    write_log(VERBOSE, "   Already got it - removing old entry.\n");
  ...
  *pp = entry;
  ...
  for (p = head; p != NULL && entry->last_access < p->last_access; p = p->next)
    ;
  entry->next = p;
...
void remove_entry(struct cache_entry *entry) {
  ...
  if (entry == hash_table[index]) {
    ...
    for (p = hash_table[index]; p->collision != NULL && p->collision != entry; p = p->collision)
      ;
    if (p->collision == NULL) {
...
sstr *get_filename(void) {
  ...
  sstr_apprintf(filename, "%02x/%08lx%03lx", cnt & 0x0F, t, (cnt >> 4) & 0xFFF);
  cnt++;
...
int init_dir(const char *dir) {
  ...
  if (stat(sstr_buf(name), &status) == -1) {
    if (mkdir(sstr_buf(name), S_IRWXU) == -1) {
      write_log(ERROR, "Unable to make cache dir %s", sstr_buf(name));
      sstr_free(name);
      return (-1);
    }
  ...
  for (i = 0; i < 16; i++) {
    ...
    if (stat(sstr_buf(name), &status) == -1) {
      write_log(IMPORT, "Making cache dir %s", sstr_buf(name));
      if (mkdir(sstr_buf(name), S_IRWXU) == -1) {
        write_log(ERROR, "Unable to make cache dir %s", sstr_buf(name));
        sstr_free(name);
        return (-1);
      ...
      write_log(ERROR, "%s is not a directory", sstr_buf(name));
      sstr_free(name);
      return (-1);
    }
  }
