...
static int sstr_cat_common(sstr * dest, const char *src, size_t len);
...
int sstr_ncpy2(sstr * dest, const char *src, size_t len)
{
	dest->len = 0;
	return sstr_cat_common(dest, src, len);
}

int sstr_ncat2(sstr * dest, const char *src, size_t len)
{
	return sstr_cat_common(dest, src, len);
}
...
int sstr_ncat(sstr * dest, const sstr * src, size_t len)
{
	if(!src)
		return 0;
	if(len > src->len)
...
size_t sstr_strtoull(const sstr * p)
{
	return (strtoull(sstr_buf(p),0,10));
}
...
int sstr_ncasecmp2(const sstr * s1, const char *s2, size_t len)
{
	if(!s2)
		return (-1);
...
int sstr_split(sstr * in, sstr * out, size_t start, size_t cnt)
{
	sstr *tmp;
	if(start + cnt > in->len )
		return (-1);
...
static int sstr_cat_common(sstr * dest, const char *src, size_t len)
{
	size_t needed_bytes = dest->len + len;
...
int sstr_alloc_space(sstr * p, size_t len)
{
	char *tmp;
